...
Feature: Interview questions
  ...
  @SC2.1
  Scenario Outline: Verify that correct answer displayed on the 'Interview Question – Selenium'
    Given Click on the "Blog" link under the image
    Then Navigate to the page "Interview Question – selenium"
    Then Find the question "<Question>"
    And Check the answer containt "<Answer>"

    Examples:
      | TestCaseID |Question | Answer |
      |1         |What is Selenium?                                                                                 |Selenium is a suite of tools for browser automation. It includes an IDE (including a recording and playback mechanism)|
      |2         |What are the limitations of the Selenium IDE? Why can’t we use it for the big automation projects?|Selenium IDE is good only for recording and playing back simple test cases.                                           |
      |3         |What is Selenium WebDriver?                                                                       |Selenium WebDriver is a tool for writing automated tests of websites.                                                 |
      |4         |What types of black box testing are the best candidates for automation?                           |Acceptance and Regression tests                                                                                       |
      |5         |What framework do you use at work?                                                                |Really up to what you know. In class we use: Selenium Webdriver/Ruby API + Cucumber to execute test cases.            |
      |6         |What is a framework?                                                                              |Collection of code that provides needed functionality                                                                 |
      |7         |How would you locate an element if it has a dynamic ID?                                           |With XPath                                                                                                            |
      |8         |What is XPath?                                                                                    |XPath is a language that describes a way to locate and process items in Extensible Markup Language (XML) documents    |
      |9         |What is the difference between absolute and relative XPaths                                       |An absolute XPath starts from the root of the DOM, and a relative XPath starts from a node that is not at the root of the DOM.|
      |10        |When is StaleElementReferenceException happens?                                                   |First of all lets be clear about what a WebElement is.A WebElement is a reference to an element in the DOM.                   |


  # @SC2.2
  # Scenario Outline: Verify that correct answer displayed on the 'Interview questions for black box testing'
  # Can't found xpath


  @SC2.3
  Scenario Outline: Verify that correct answer displayed on the 'Interview questions for black box testing – page 2'
    Given Click on the "Blog" link under the image
